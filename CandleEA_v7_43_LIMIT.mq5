//+------------------------------------------------------------------+
//|     CandleEA v7.43 - FULL PROTECTION (PASSWORD + EXPIRY + ID)    |
//|     ✅ NEW: Bảo mật Account ID (User KHÔNG thấy)                 |
//|     ✅ NEW: Bảo mật mật khẩu                                     |
//|     ✅ NEW: Hạn sử dụng ẨN                                       |
//|     ✅ NEW: TP cố định cho MAIN (có thể bật/tắt)                |
//|     ✅ NEW: Extra lot cho từng hedge level (L0-L19)             |
//|     ✅ NEW: TP Money check TỔNG profit của TẤT CẢ lệnh          |
//|     ✅ Khi hedge khớp → XÓA TP MAIN, quay về logic cũ           |
//|     🔁 CHANGE: MAIN & HEDGE pending từ STOP → LIMIT             |
//+------------------------------------------------------------------+
#property strict
...
//+------------------------------------------------------------------+
//| ========== BẢO MẬT - CHỈ BẠN THẤY TRONG CODE ==========         |
//+------------------------------------------------------------------+
input group "=== LICENSE PROTECTION ==="
input string PASSWORD = "369369K";  // Nhập mật khẩu để kích hoạt EA

// ✅ CÁC THÔNG TIN BẢO MẬT - User KHÔNG thấy trong MT5
const datetime EXPIRY_DATE = D'2026.01.12 23:59:59';  // Ngày hết hạn EA
const string CORRECT_PASSWORD = "369369K";           // Mật khẩu đúng
const long AUTHORIZED_ACCOUNT = 11360582;              // Account ID được phép

//+------------------------------------------------------------------+
//| ========== CÁC THIẾT LẬP TRADING ==========                     |
//+------------------------------------------------------------------+
...
// ✅✅✅ THÊM MỚI: Khối lượng bổ sung cho mỗi hedge level
input group "=== HEDGE EXTRA LOTS (CỘNG VÀO BASE LOT) ==="
input double InpHedgeL0ExtraLot  = 0.0;  // L0 - Lot bổ sung
input double InpHedgeL1ExtraLot  = 0.0;  // L1 - Lot bổ sung
input double InpHedgeL2ExtraLot  = 0.0;  // L2 - Lot bổ sung
input double InpHedgeL3ExtraLot  = 0.0;  // L3 - Lot bổ sung
input double InpHedgeL4ExtraLot  = 0.0;  // L4 - Lot bổ sung
input double InpHedgeL5ExtraLot  = 0.0;  // L5 - Lot bổ sung
input double InpHedgeL6ExtraLot  = 0.0;  // L6 - Lot bổ sung
input double InpHedgeL7ExtraLot  = 0.0;  // L7 - Lot bổ sung
input double InpHedgeL8ExtraLot  = 0.0;  // L8 - Lot bổ sung
input double InpHedgeL9ExtraLot  = 0.0;  // L9 - Lot bổ sung
input double InpHedgeL10ExtraLot = 0.0;  // L10 - Lot bổ sung
input double InpHedgeL11ExtraLot = 0.0;  // L11 - Lot bổ sung
input double InpHedgeL12ExtraLot = 0.0;  // L12 - Lot bổ sung
input double InpHedgeL13ExtraLot = 0.0;  // L13 - Lot bổ sung
input double InpHedgeL14ExtraLot = 0.0;  // L14 - Lot bổ sung
input double InpHedgeL15ExtraLot = 0.0;  // L15 - Lot bổ sung
input double InpHedgeL16ExtraLot = 0.0;  // L16 - Lot bổ sung
input double InpHedgeL17ExtraLot = 0.0;  // L17 - Lot bổ sung
input double InpHedgeL18ExtraLot = 0.0;  // L18 - Lot bổ sung
input double InpHedgeL19ExtraLot = 0.0;  // L19 - Lot bổ sung
...
//+------------------------------------------------------------------+
//| ✅ STRUCT LƯU THÔNG TIN HEDGE                                   |
//+------------------------------------------------------------------+
struct HedgeInfo
{
   ulong ticket;
   ulong originalPendingTicket;
   ENUM_ORDER_TYPE type;
   ENUM_HEDGE_STATE state;
   double lot;
   double price;
   double sl;
   double tp;
...
bool CheckPassword()
{
   if(IsBacktesting())
   {
      Print("BACKTEST MODE - Bypass password check");
      return true;
   }
...
bool CheckExpiry()
{
   if(IsBacktesting())
   {
      Print("BACKTEST MODE - Bypass expiry check");
      return true;
   }
...
bool CheckAccountID()
{
   if(IsBacktesting())
   {
      Print("BACKTEST MODE - Bypass account ID check");
      return true;
   }

   long currentAccount = AccountInfoInteger(ACCOUNT_LOGIN);

   if(currentAccount != AUTHORIZED_ACCOUNT)
      return false;
...
{
   datetime now = TimeCurrent();
   int seconds = (int)(EXPIRY_DATE - now);
   return seconds / 86400;
}

//+------------------------------------------------------------------+
//| ✅✅✅ TÍNH LOT CHO HEDGE LEVEL (BASE * MULTIPLIER^LEVEL + EXTRA)|
//+------------------------------------------------------------------+
double GetHedgeLotForLevel(int level)
{
   // Tính base lot theo martingale
   double baseLot = InpHedgeLotSize;
   for(int i = 0; i < level; i++)
   {
      baseLot *= InpHedgeMultiplier;
   }

   // Lấy extra lot tương ứng với level
   double extraLot = 0.0;

   switch(level)
   {
      case 0:  extraLot = InpHedgeL0ExtraLot;  break;
      case 1:  extraLot = InpHedgeL1ExtraLot;  break;
      case 2:  extraLot = InpHedgeL2ExtraLot;  break;
      case 3:  extraLot = InpHedgeL3ExtraLot;  break;
      case 4:  extraLot = InpHedgeL4ExtraLot;  break;
      case 5:  extraLot = InpHedgeL5ExtraLot;  break;
      case 6:  extraLot = InpHedgeL6ExtraLot;  break;
      case 7:  extraLot = InpHedgeL7ExtraLot;  break;
      case 8:  extraLot = InpHedgeL8ExtraLot;  break;
      case 9:  extraLot = InpHedgeL9ExtraLot;  break;
      case 10: extraLot = InpHedgeL10ExtraLot; break;
      case 11: extraLot = InpHedgeL11ExtraLot; break;
      case 12: extraLot = InpHedgeL12ExtraLot; break;
      case 13: extraLot = InpHedgeL13ExtraLot; break;
      case 14: extraLot = InpHedgeL14ExtraLot; break;
      case 15: extraLot = InpHedgeL15ExtraLot; break;
      case 16: extraLot = InpHedgeL16ExtraLot; break;
      case 17: extraLot = InpHedgeL17ExtraLot; break;
      case 18: extraLot = InpHedgeL18ExtraLot; break;
      case 19: extraLot = InpHedgeL19ExtraLot; break;
      default: extraLot = 0.0; break;
   }

   // Tổng lot = base + extra
   double totalLot = baseLot + extraLot;
   totalLot = NormalizeDouble(totalLot, 2);

   PrintFormat("📊 HEDGE L%d: Base %.2f + Extra %.2f = TOTAL %.2f",
              level, baseLot, extraLot, totalLot);
...
//+------------------------------------------------------------------+
//| ✅ KIỂM TRA CÓ HEDGE ACTIVE TRONG LIST                          |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| ✅ KIỂM TRA CÓ HEDGE ACTIVE TRONG MT5                           |
//+------------------------------------------------------------------+
bool HasActiveHedgeTypeInMT5(ENUM_ORDER_TYPE hedgeType)
{
   ...
   // LIMIT mapping: BUY_LIMIT -> BUY position, SELL_LIMIT -> SELL position
   ENUM_POSITION_TYPE posType = (hedgeType == ORDER_TYPE_BUY_LIMIT) ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;
...
int OnInit()
{
   ...
   {
      Alert("❌ SAI MẬT KHẨU!");
      Alert("EA bị khóa. Vui lòng nhập đúng mật khẩu.");
      Comment("🔒 EA BỊ KHÓA - Sai mật khẩu");
      Print("==========================================================");
      Print("❌ EA KHÔNG KHỞI ĐỘNG - SAI MẬT KHẨU");
      Print("==========================================================");
      return(INIT_FAILED);
   }
   ...
   {
      Alert("❌ EA ĐÃ HẾT HẠN!");
      Alert("Hạn sử dụng: ", TimeToString(EXPIRY_DATE, TIME_DATE));
      Alert("Vui lòng liên hệ để gia hạn.");
      Comment("🔒 EA HẾT HẠN\nHạn: " + TimeToString(EXPIRY_DATE, TIME_DATE));
      Print("==========================================================");
      Print("❌ EA ĐÃ HẾT HẠN - Ngày hết hạn: ", TimeToString(EXPIRY_DATE, TIME_DATE|TIME_SECONDS));
      Print("==========================================================");
      return(INIT_FAILED);
   }
   ...
   if(!CheckAccountID())
   {
      ...
      Alert("Account của bạn: ", currentAccount);
      Alert("EA chỉ hoạt động trên account đã đăng ký.");
      Alert("Vui lòng liên hệ để đăng ký account này.");
      Comment("🔒 EA BỊ KHÓA - Account không hợp lệ");
      Print("==========================================================");
      Print("❌ ACCOUNT KHÔNG HỢP LỆ");
      PrintFormat("Account hiện tại: %I64d", currentAccount);
      PrintFormat("Account được phép: %I64d", AUTHORIZED_ACCOUNT);
      Print("==========================================================");
      return(INIT_FAILED);
   }
   ...
   {
      Print("==========================================================");
      Print("🧪 BACKTEST MODE - Security checks bypassed");
      Print("==========================================================");
   }
   else
   {
      ...
      Alert("✅ EA đã kích hoạt thành công!");
      Alert("Account: ", currentAccount);

      if(daysLeft <= 7)
         Alert("⚠️ CẢNH BÁO: EA còn ", daysLeft, " ngày sử dụng!");
      else if(daysLeft <= 30)
         Alert("ℹ️ EA còn ", daysLeft, " ngày sử dụng");

      Print("==========================================================");
      Print("✅ ALL CHECKS PASSED - EA ACTIVATED");
      PrintFormat("📅 Hạn sử dụng: %s (Còn %d ngày)",
                 TimeToString(EXPIRY_DATE, TIME_DATE), daysLeft);
      PrintFormat("👤 Account: %I64d", currentAccount);
   }

   Print("==========================================================");
   Print("CandleEA v7.43 - FULL PROTECTION + EXTRA LOTS");
   Print("- Password Protection (HIDDEN)");
   Print("- Expiry Date Protection (HIDDEN)");
   Print("- Account ID Protection (HIDDEN)");
   Print("- Extra Lot cho moi Hedge Level (L0-L19)");
   Print("- TP Money check TONG profit tat ca lenh");
   Print("- Auto bypass security khi backtest");
   Print("- TP co dinh cho MAIN (co the bat/tat)");
   Print("- MAIN/HEDGE pending: LIMIT (BUY_LIMIT/SELL_LIMIT)");
   Print("==========================================================");
   ...
   if(IsBacktesting())
      Comment("🧪 BACKTEST MODE");
   else
      Comment(StringFormat("✅ EA ĐANG CHẠY\nCòn %d ngày", GetDaysRemaining()));
...
{
   // ========================================================
   // ✅✅✅ KIỂM TRA BẢO MẬT MỖI 3 GIỜ
   // ========================================================
   static datetime lastSecurityCheck = 0;
   datetime now = TimeCurrent();

   if(lastSecurityCheck == 0 || now - lastSecurityCheck >= 10800)
   {
      if(!CheckPassword())
      {
         Comment("🔒 EA BỊ KHÓA - Sai mật khẩu");
      ...
      {
         long currentAccount = AccountInfoInteger(ACCOUNT_LOGIN);
         Comment(StringFormat("🔒 EA BỊ KHÓA\nAccount: %I64d\nKhông hợp lệ", currentAccount));
         return;
      }
      ...
      {
         int daysLeft = GetDaysRemaining();
         PrintFormat("✅ Security check OK - Còn %d ngày", daysLeft);
      }
   }

   // ========================================================
   // PHẦN LOGIC TRADING BÊN DƯỚI GIỮ NGUYÊN 100%
   // (Chỉ đổi STOP -> LIMIT ở phần mở lệnh & hedge)
   // ========================================================
...
//+------------------------------------------------------------------+
// TẤT CẢ CÁC FUNCTION TRADING BÊN DƯỚI
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| ✅✅✅ ĐỔI MAIN PENDING: BUY_STOP/SELL_STOP -> BUY_LIMIT/SELL_LIMIT|
//| Lưu ý: LIMIT hợp lệ => BUY_LIMIT đặt dưới giá, SELL_LIMIT đặt trên|
//+------------------------------------------------------------------+
void PlaceBreakoutOrders()
{
   ...
   // LIMIT logic:
   // - BUY_LIMIT: đặt dưới vùng lowPrev
   // - SELL_LIMIT: đặt trên vùng highPrev
   double buyPrice  = NormalizeDouble(lowPrev  - (2 + InpDistancePips) * pipValue, _Digits);
   double sellPrice = NormalizeDouble(highPrev + (2 + InpDistancePips) * pipValue, _Digits);
   ...
   PlacePendingOrder(ORDER_TYPE_BUY_LIMIT,  buyPrice,  InpLotSize, slBuy,  tpBuy,  MAGIC_MAIN, "MAIN", -1);
   PlacePendingOrder(ORDER_TYPE_SELL_LIMIT, sellPrice, InpLotSize, slSell, tpSell, MAGIC_MAIN, "MAIN", -1);
}

//+------------------------------------------------------------------+
//| ✅✅✅ QUẢN LÝ LỆNH MỞ - CHECK TỔNG PROFIT                       |
//+------------------------------------------------------------------+
void ManageOpenPosition()
{
   // ========================================================
   // ✅✅✅ KIỂM TRA TỔNG PROFIT CỦA TẤT CẢ LỆNH
   // ========================================================
   if(InpTakeProfitMoney > 0)
   {
      ...
      // ✅ NẾU TỔNG PROFIT ĐẠT MỤC TIÊU → ĐÓNG TẤT CẢ
      if(totalProfit >= InpTakeProfitMoney)
      {
         Print("========================================================");
         PrintFormat("💰 TỔNG PROFIT ĐẠT MỤC TIÊU!");
         PrintFormat("Tổng profit: %.2f USD (Mục tiêu: %.2f USD)", totalProfit, InpTakeProfitMoney);
         PrintFormat("Số lệnh: %d", positionCount);
         Print("→ ĐÓNG TẤT CẢ VÀ BẮT ĐẦU LẠI");
         Print("========================================================");
   ...
   // ========================================================
   // ✅ PHẦN CÒN LẠI: BREAK EVEN CHO TỪNG LỆNH
   // ========================================================
...
void DeleteHedgeOrders(ENUM_POSITION_TYPE closedType)
{
   ...
   // LIMIT mapping: main BUY đóng -> xóa SELL_LIMIT hedge; main SELL đóng -> xóa BUY_LIMIT hedge
   ENUM_ORDER_TYPE targetType = (closedType == POSITION_TYPE_BUY) ? ORDER_TYPE_SELL_LIMIT : ORDER_TYPE_BUY_LIMIT;
...
bool RecoverHedgeExact(HedgeInfo &info, string reason)
{
   ...
   // LIMIT: BUY_LIMIT đặt dưới ask, SELL_LIMIT đặt trên bid
   double currentPrice = (info.type == ORDER_TYPE_BUY_LIMIT) ? ask : bid;
   double distancePips = 0;

   if(info.type == ORDER_TYPE_BUY_LIMIT)
      distancePips = (ask - info.price) / pipValue;
   else
      distancePips = (info.price - bid) / pipValue;
   ...
   ENUM_ORDER_TYPE marketType = (info.type == ORDER_TYPE_BUY_LIMIT) ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
...
void CheckAndRecoverMissingHedge()
{
   ...
   if(hasMainBuy)
   {
      if(HasActiveHedgeType(ORDER_TYPE_SELL_LIMIT))
      {
         return;
      }
      else
      {
         ...
         double currentHedgeLot = GetHedgeLotForLevel(hedgeLevel);

         HedgeInfo recoveryInfo;
         recoveryInfo.type = ORDER_TYPE_SELL_LIMIT;
   ...
   if(hasMainSell)
   {
      if(HasActiveHedgeType(ORDER_TYPE_BUY_LIMIT))
      {
         return;
      }
      else
      {
         ...
         HedgeInfo recoveryInfo;
         recoveryInfo.type = ORDER_TYPE_BUY_LIMIT;
         recoveryInfo.lot = currentHedgeLot;
         recoveryInfo.price = hedgePrice;
         recoveryInfo.sl = sl;
         recoveryInfo.tp = tp;
                        ...
                        const MqlTradeResult &result)
{
   ...
   if(deal_magic == MAGIC_MAIN && entryType == DEAL_ENTRY_IN)
   {
      ...
      if(InpUseHedge)
      {
         ...
         // LIMIT mapping:
         // - Main BUY -> đặt SELL_LIMIT phía trên
         // - Main SELL -> đặt BUY_LIMIT phía dưới
         ENUM_ORDER_TYPE hedgeType = (dealType == DEAL_TYPE_BUY) ? ORDER_TYPE_SELL_LIMIT : ORDER_TYPE_BUY_LIMIT;
         ...
         else
         {
            ...
            double hedgeLot = GetHedgeLotForLevel(0);
            ...
            if(dealType == DEAL_TYPE_BUY)
            {
               ...
               PlacePendingOrder(ORDER_TYPE_SELL_LIMIT, sellPrice,
                               hedgeLot, sl, tp, MAGIC_HEDGE, "HEDGE-L0", 0);
            }
            else
            {
               ...
               PlacePendingOrder(ORDER_TYPE_BUY_LIMIT, buyPrice,
                               hedgeLot, sl, tp, MAGIC_HEDGE, "HEDGE-L0", 0);
            }
   ...
   else if(deal_magic == MAGIC_HEDGE && entryType == DEAL_ENTRY_IN)
   {
      ...
      double nextLot = GetHedgeLotForLevel(hedgeLevel);
      ...
      // LIMIT chain:
      // - Nếu hedge vừa khớp BUY -> đặt tiếp SELL_LIMIT phía trên
      // - Nếu hedge vừa khớp SELL -> đặt tiếp BUY_LIMIT phía dưới
      if(dealType == DEAL_TYPE_BUY)
      {
         double sellPrice = NormalizeDouble(openPrice + InpHedgeDistancePips * pipValue, _Digits);
         double sl = NormalizeDouble(sellPrice + InpHedgeStopLossPips * pipValue, _Digits);
         ...
         PlacePendingOrder(ORDER_TYPE_SELL_LIMIT, sellPrice, nextLot, sl, tp,
      ...
      else
      {
         ...
         PlacePendingOrder(ORDER_TYPE_BUY_LIMIT, buyPrice, nextLot, sl, tp,
                         MAGIC_HEDGE, StringFormat("HEDGE-L%d", hedgeLevel), hedgeLevel);
      }
